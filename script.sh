...
# Функция для копирования отсутствующих подпапок из preprod в perf для POD
copy_missing_pod_subfolders() {
    local preprod_folder="$1"
    local perf_folder="$2"

    # Получаем список подпапок внутри POD в preprod и perf
    local preprod_pod_subfolders
    local perf_pod_subfolders

    preprod_pod_subfolders=$(find "$preprod_folder" -mindepth 1 -maxdepth 1 -type d)
    perf_pod_subfolders=$(find "$perf_folder" -mindepth 1 -maxdepth 1 -type d)

    # Проходим по всем подпапкам preprod
    for preprod_pod_subfolder in $preprod_pod_subfolders; do
        local relative_subfolder="${preprod_pod_subfolder#$preprod_folder/}"  # Относительный путь подпапки
        local perf_pod_subfolder="$perf_folder/$relative_subfolder"  # Путь к подпапке в perf

        # Проверяем, есть ли такая подпапка в perf
        if [[ ! -d "$perf_pod_subfolder" ]]; then
            echo "Подпапка $relative_subfolder не найдена в $perf_folder. Копируем..."
            mkdir -p "$perf_pod_subfolder"  # Создаем отсутствующую подпапку в perf
            cp -r "$preprod_pod_subfolder"
            cp -r "$preprod_pod_subfolder" "$perf_pod_subfolder"  # Копируем содержимое из preprod в perf
            echo "Подпапка $relative_subfolder и ее содержимое успешно скопированы в $perf_folder."
        else
            echo "Подпапка $relative_subfolder уже существует в $perf_folder, пропускаем."
        fi
    done
}
...
main() {
    ...
    copy_missing_pod_subfolders "$PREPROD_DIR" "$PERF_DIR"
